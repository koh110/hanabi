...
(function(DATA, base64) {
  const CANVAS_WIDTH = 433;
  const CANVAS_HEIGHT = 323;
  var getData = (function() {
    const threshold = 128; // しきい値
    const reductionRate = 10; // データ削減率
    var convertGrayScale = function(base64Data) {
      var img = new Image;
      img.src = base64Data;
      ...
      let cpa = ctx.getImageData(0, 0, W, H).data;
  ...
  var renderHanabi = function(dataArr) {
    var cvs = document.createElement('canvas'),
    ...
    let requestAnimationFrame = window.webkitRequestAnimationFrame
      || window.mozRequestAnimationFrame
      || window.msRequestAnimationFrame
    ...
    function render() {
      ...
      if (per < 100) {
        ...
        for (i = d.length; i--; ) {
          p = d[i];
          c.fillRect(
            p[0] * RADIUS * per / 10000,
            p[1] * RADIUS * per / 10000,
            1, 1
          );
        }
        c.restore();
